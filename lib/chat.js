...
var replaceEmoticons = function(text, username){
	text = text.replace(/(http(s?):\/\/([^ ]+))/ig, "<a href=\"$1\" target=\"_external\">$1</a>");

	if(typeof(username) != "undefined") {
		var regex = new RegExp("(@"+username+")", "gi");
		if(text.match(regex)) gui.Window.get().requestAttention(3);
		text = text.replace(regex, '<span class="nick">$1</span>');
	}

	if(emoticons == null) return text;
	for(var i = 0; i < emoticons.emoticons.length; i++) {
		var e = emoticons.emoticons[i];
		if(e.images.length > 0) {
			var ereg = e.regex.replace(/</g, "\\&lt;").replace(/>/g, "\\&gt;").replace(/\&/g, "\\&amp;");
			var regex = new RegExp("( |^)"+e.regex+"($| )", 'g');
			text = text.replace(regex, ' <img src="'+e.images[0].url+'" style="height:'+e.images[0].height+'px;width:'+e.images[0].width+'px" alt=""> ');
		} else {
			console.log(e.regex, "got no images?");
			console.log(e);
		}
	}
...
var refreshUserList = function(channel, users, colors, mods){
	if(doNotRefresh) {
		if(!refreshing) {
			refreshing = true;
			window.setTimeout(function(){ doNotRefresh = false; refreshing = false; refreshUserList(channel, users, colors, mods); }, 3000);
		}
	} else {
		...
		for(var i = 0; i < users.length; i++) {
			var u = users[i];
			var icon = '';
			if(mods.indexOf(u) >= 0) icon = '<span class="icon-sword"></span> ';
			if(u == channel) icon = '<span class="icon-video-camera"></span> ';
			var c = getColor(u);
...
var TwitchChat = (function(){
	function TwitchChat(userinfo, token) {
		...
		this.irc.on("join", function(channel, nick, msg){
			...
			refreshUserList(channel, _this.users[channel], _this.colors, _this.mods[channel]);
		});
		...
		this.irc.on("message", function(nick, to, text, msg){
			if(to.substr(0, 1) == "#") {
				...
				if(_this.mods[to].indexOf(nick) >= 0) icon = '<span class="icon-sword"></span> ';
				if(nick == to) icon = '<span class="icon-video-camera"></span> ';
				text = replaceEmoticons(text, _this.userinfo.name);
				var c = getColor(nick);
				if(typeof(_this.colors[nick]) != "undefined") c = _this.colors[nick];

				$("#"+to).append($("<div />").addClass("message").html('['+timestamp()+'] '+icon+'<span class="nick" style="color:'+c+'">'+nick+'</span>: '+text));
				...
				if(_this.users[to].indexOf(nick) < 0) {
					...
					if(typeof(_this.mods[to]) == "undefined") _this.mods[to] = [];
					refreshUserList(to, _this.users[to], _this.colors, _this.mods[to]);
				}
			}
		});
		this.irc.on("action", function(from, to, text, msg){
			if(to.substr(0, 1) == "#") { // Yep, it's a channel
				...
				var icon = '';
				if(typeof(_this.mods[to]) == "undefined") _this.mods[to] = [];
				if(_this.mods[to].indexOf(from) >= 0) icon = '<span class="icon-sword"></span> ';
				if(from == to) icon = '<span class="icon-video-camera"></span> ';
				text = replaceEmoticons(text, _this.userinfo.name);
				var c = getColor(from);
		...
		this.irc.on("raw", function(msg){
			if(msg.command == "PRIVMSG" && msg.nick == "jtv") {
				if(msg.args[1].substr(0, 9) == "USERCOLOR") {
					var a = msg.args[1].split(" ", 3);
					_this.colors[a[1]] = a[2];

					$.each(_this.users, function(c, v){
						$.each(v, function(i, n){
							if(n == a[1]) {
								if(typeof(_this.users[c]) == "undefined") _this.users[c] = [];
								if(typeof(_this.mods[c]) == "undefined") _this.mods[c] = [];
								refreshUserList(c, _this.users[c], _this.colors, _this.mods[c]);
							}
						});
					});
				}
			...
			if(msg.command != "PRIVMSG") {
				var scrolltobottom = scrolledToBottom();
				$("#server").append($("<div />").addClass("message").html("["+timestamp()+"] "+(typeof(msg.prefix) != "undefined" ? msg.prefix +" " : "")+msg.rawCommand+" "+msg.args.join(" ")));
	...
	TwitchChat.prototype.join = function(channel, joined){
		...
		$("#navigation").append('<li class="tab-title" role="presentational" ><a href="#'+channel+'" role="tab" tabindex="0" aria-selected="false" controls="'+channel+'" id="opentab'+channel+'">#'+channel+'</a></li>');
		...
		window.setTimeout(function(){
			...
			$("#msg"+channel).keyup(function(e){
				...
				if(e.which == 13) {
					...
					_this.irc.emit("message", n, "#"+c, message, {
						prefix: n+".tmi.twitch.tv",
						nick: n,
						user: n,
						host: n+".tmi.twitch.tv",
						server: n+".tmi.twitch.tv",
