...
var Chat = (function(){
	function Chat(userinfo, token) {
		...
		this.onwhisper = function(prefix, nick, to, text, tags){
			...
			manageTags(_this, tags, nick, "usr_"+to);
	...
	Chat.prototype.joinWhisper = function(user){
		...
		$("#opentabserver").parent().after('<li class="tab-title user" role="presentational" ><a href="#usr_'+user+'" role="tab" tabindex="0" aria-selected="false" controls="usr_'+user+'" id="opentabusr_'+user+'">@'+user+'</a></li>');
		$("#content").append('<section role="tabpanel" aria-hidden="true" class="content channel" id="usr_'+user+'"><input type="text" id="msgusr_'+user+'" class="postmessage" placeholder="'+getTranslation("send_a_message")+'"><a class="emoticonbtn" id="emoteusr_'+user+'"><span class="icon-smile"></span></a><div class="emoticons" id="emotesusr_'+user+'"></div></section>');


		$("#usr_"+user).bind("DOMSubtreeModified", function(){
			if($("#opentabusr_"+user).length) {
				if($("#opentabusr_"+user).parent().is(".active") && scrolledToBottom) {
					$(document).scrollTop($(document).height());
					$("img").load(function(){ $(this).unbind(); $(document).scrollTop($(document).height()); });
				}
			}
		...
		window.setTimeout(function(){
			...
			$("#opentabusr_"+user).mouseup(function(e){
				...
				if(e.which == 2) {
					...
					var menu = new gui.Menu();
					var mi_close = new gui.MenuItem({ label: getTranslation("close_channel"), click: function(){ $("#usr_"+c).remove();$("#opentabusr_"+c).parent().remove(); } });
					menu.append(mi_close);
					menu.popup(e.clientX, e.clientY);
	...
	Chat.prototype.sendWhisper = function(to, message) {
		...
		this.whisperirc.say("#jtv", "/w "+to+" "+message);
