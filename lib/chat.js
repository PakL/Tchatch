...
var autoRefreshStatus = function(chat, channels) {
...
var TwitchChat = (function(){
		...
		this.irc.on("raw", function(msg){
			if(msg.command == "PRIVMSG" && msg.nick == "jtv") {
				if(msg.args[1].substr(0, 9) == "USERCOLOR") {
					...
					if(_this.colors.indexOf(a[1]) < 0 || _this.colors[a[1]] != a[2]) {
						_this.colors[a[1]] = a[2];

						$.each(_this.channels, function(ic, c){
							$.each(_this.users[c], function(i, n){
								if(n == a[1]) {
									refreshUserList(c, _this);
								}
							});
						});
					}
				} else if(msg.args[1].substr(0, 11) == "SPECIALUSER"){
					var a = msg.args[1].split(" ", 3);
					if(a[2] == "subscriber") {
						_this.subwait.push(a[1]);
					} else if(a[2] == "turbo") {
						if(_this.turbo.indexOf(a[1]) < 0) {
							_this.turbo.push(a[1]);
						}
					}
				}
			}
	...
	TwitchChat.prototype.refreshStatus = function(channel, cb){
		var _this = this;
		https.get({
			"method": "GET", "host": "api.twitch.tv", "path": "/kraken/channels/"+channel,
			"headers": { "Accept": "application/vnd.twitchtv.v3+json" }
		}, function(resp){
			var data = "";
			resp.on("data", function(chunk){
				...
				if(resp.statusCode == 200) {
					...
					_this.channelstatus[channel] = channeldata.status;
					_this.channelgame[channel] = channeldata.game;
