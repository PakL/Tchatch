...
var replaceEmoticons = function(text, username){
	...
	for(var i = 0; i < highlights.length; i++) {
		...
		if(text.match(regex)) {
			...
			switch(highlights[i].type) {
				case "1":
					text = text.replace(regex, function(needle){ return '<span style="font-weight:bold;'+(highlights[i].color.length > 0 ? "color:"+highlights[i].color+";" : "")+'">'+needle+'</span>'; });
					break;
				case "2":
	...
	for(var i = 0; i < emoticons.emoticons.length; i++) {
		...
		if(e.images.length > 0) {
			...
			var regex = new RegExp("(\s|^)"+e.regex+"($|\s)", 'g');
