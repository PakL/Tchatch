...
var replaceEmoticons = function(text, username, emotes){
	if(typeof(emotes) == "string") {
		...
		for(var i = 0; i < e.length; i++) {
			...
			if(splits.length == 2) {
				...
				for(var j = 0; j < ranges.length; j++) {
					...
					if(indexes.length == 2)
						replacings.push({ "id": eid, "start": parseInt(indexes[0]), "end": parseInt(indexes[1]) });
				}
			}
		}
		...
		for(var i = 0; i < replacings.length; i++) {
			...
			newtext += '<img src="http://static-cdn.jtvnw.net/emoticons/v1/'+replacings[i].id+'/1.0" alt="'+text.substring(replacings[i].start, replacings[i].end+1)+'" title="'+text.substring(replacings[i].start, replacings[i].end+1)+'">';
	...
	//TODO: Hier wird verdammt viel schiefgehen, wenn ich das nicht anders löse...
...
var TwitchChat = (function(){
	function TwitchChat(userinfo, token) {
		...
		this.onuserstate = function(tags) {
		...
		this.irc.on("raw", function(msg){
			...
			if(pmsg.command == "PRIVMSG" && pmsg.params[0].substr(0, 1) == "#") {
				...
				_this.onuserstate(pmsg.tags);
			} else if(pmsg.command == "NOTICE") {
				//TODO: Notice an den Channel posten
			}
