...
var appendToChannel = function(chat, channel, html) {
	if($("#"+channel).length) {
		$("#"+channel).append(html);
		if(typeof(localStorage.history) != "undefined" && localStorage.history == "true")
...
var TwitchChat = (function(){
	function TwitchChat(userinfo, token) {
		...
		this.irc.on("+mode", function(channel, by, mode, argument, msg){
			channel = channel.substr(1);
			var appending = $("<div />").addClass("message").addClass("server").html('['+timestamp()+'] '+util.format(getTranslation("got_mod"), '<span class="nick">'+msg.args[2]+'</span>'));
			appendToChannel(_this, channel, appending.prop('outerHTML'));
		...
		this.irc.on("-mode", function(channel, by, mode, argument, msg){
			channel = channel.substr(1);
			var appending = $("<div />").addClass("message").addClass("server").html('['+timestamp()+'] '+util.format(getTranslation("remove_mod"), '<span class="nick">'+msg.args[2]+'</span>'));
			appendToChannel(_this, channel, appending.prop('outerHTML'));
