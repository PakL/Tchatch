...
var appendToChannel = function(chat, channel, html) {
	if($("#"+channel).length) {
		$("#"+channel).append(html);
		if(typeof(window.localStorage.history) != "undefined" && window.localStorage.history == "true")
...
var Chat = (function(){
	function Chat(userinfo, token) {
		...
		this.irc.on("registered", function(msg){
			...
			if(typeof(window.localStorage.reopen) != "undefined") {
				var reopen = JSON.parse(window.localStorage.reopen);
	...
	Chat.prototype.disconnect = function(cb){
		...
		window.localStorage.reopen = JSON.stringify(_this.channels);
