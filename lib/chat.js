...
var replaceEmoticons = function(text, username){
	...
	for(var i = 0; i < emoticons.emoticons.length; i++) {
		...
		if(e.images.length > 0) {
			...
			text = text.replace(regex, ' <img src="'+e.images[0].url+'" alt=""> ');
		} else {
	...
	function TwitchChat(userinfo, token) {
		...
		this.irc.on("join", function(channel, nick, msg){
			...
			if($("#opentab"+channel).length) {
				if($("#opentab"+channel).parent().is(".active") && scrolltobottom) $(document).scrollTop($(document).height());
		...
		this.irc.on("message", function(nick, to, text, msg){
			if(to.substr(0, 1) == "#") {
				...
				text = text.replace(/\&/g, "&aml;").replace(/\</g, "&lt;").replace(/\>/g, "&gt;");
				var icon = '';
				...
				if($("#opentab"+to).length) {
					if($("#opentab"+to).parent().is(".active") && scrolltobottom) $(document).scrollTop($(document).height());
