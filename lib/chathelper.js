...
var replaceEmoticons = function(text, username, emotes){
	...
	if(textWoe.match(regex) && text.match(regex)) {
		while(match = regex.exec(text)) {
			replacings.push({ "replaceWith": match[1] + "<a href=\"" + match[2] + "\" target=\"_external\">" + match[2] + "</a>" + match[5], "start": match.index, "end": match.index+match[0].length });
		}
	}
