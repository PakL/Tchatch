	...
	https.get({
		...
		resp.on("data", function(chunk){
			...
			if(resp.statusCode == 200) {
				fs.writeFile("emoticons.json", data, function (err) {
					if(!err) {
...
$(document).ready(function(){
	...
	$("#navigation").on('toggled', function (event, tab) {
		...
		if(opentab != "start") {
			...
			if(opentab != "server") {
				$("footer .button").removeClass("disabled").click(function(){
				...
				$("footer .button").addClass("disabled").unbind('click');
			}
		} else {
			$("footer .button").addClass("disabled").unbind('click');
		}
	...
	if(fs.existsSync("emoticons.json")) {
		fs.readFile("emoticons.json", function(err, data){
